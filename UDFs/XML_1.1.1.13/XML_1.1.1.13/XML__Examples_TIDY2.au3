...
_Example_TIDY(@ScriptDir & "\XML_Example_XML_Files\TFJ_TIDY.xml")

#Region XML__Examples.au3 - Function

Func _Example_TIDY($sXML_FileFullPath)
...
;~ 	Local $oXMLDoc = _XML_CreateDOMDocument(Default)
	...
	Else
		...
		Else
			...
			MsgBox(0, 'TEST', $oXMLDoc.xml)
			Local $sXmlAfterTidy = _XML_TIDY($oXMLDoc, -1)
			If @error Then
				MsgBox(0, '_XML_TIDY @error:', XML_My_ErrorParser(@error))
			Else
				MsgBox($MB_SYSTEMMODAL + $MB_ICONINFORMATION, '_Example_TIDY', $sXmlAfterTidy)
			EndIf
		EndIf
	EndIf
EndFunc   ;==>_Example_TIDY
